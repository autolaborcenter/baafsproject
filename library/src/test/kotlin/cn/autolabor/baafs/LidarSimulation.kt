...
fun AnalyticalShape.toPolygon(where: Vector2D) =
    sample().toList().map { where + it }.let(::Polygon)
...
fun main() = runBlocking(Dispatchers.Default) {
    ...
    val obstacles =
        List(10) { i ->
            listOf(Circle(.14, 32).toPolygon(vector2DOf(i * .3, +.5)),
                   Circle(.14, 32).toPolygon(vector2DOf(i * .3, -.5)))
        }.flatten()
