...
fun File.loadAsScan(): List<Vector2D> {
    ...
    val (x0, y0) = when (bytes.size - (bytesPerRow * height + 54)) {
        0                  -> 0 to 0
        2 * Int.SIZE_BYTES -> {
            val i = bytes.size - 2 * Int.SIZE_BYTES
            buffer.getInt(i) to buffer.getInt(i + Int.SIZE_BYTES)
        }
        else               -> throw IllegalArgumentException("error format in bmp file")
    }
