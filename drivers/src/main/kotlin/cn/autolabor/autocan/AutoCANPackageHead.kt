...
internal sealed class AutoCANPackageHead(
    ...
    class PackWithData(
        ...
        nodeIndex: Byte,
        messageType: Byte
    ) : AutoCANPackageHead(network, priority, nodeType, nodeIndex, messageType) {
        private val stub = buildStub(true)
        fun pack(data: ByteArray = byteArrayOf()): ByteArray {
            require(data.size <= 8)
            return stub.copyOf().apply {
                data.copyInto(this, 6)
                this[lastIndex] = copyOfRange(1, lastIndex).calculateCrc8().toByte()
            }
        }
    }

    class PackWithoutData(
        network: Byte,
        priority: Byte,
        nodeType: Byte,
        nodeIndex: Byte,
        ...
        fun pack(reserve: Byte = 0): ByteArray =
            stub.copyOf().apply {
                this[lastIndex - 1] = reserve
                this[lastIndex] = copyOfRange(1, lastIndex).calculateCrc8().toByte()
            }
    }

    protected fun buildStub(dataField: Boolean) =
        ByteArray(if (dataField) 14 else 6)
            .also {
                buildSerial4Bytes {
                    write(8, 0xfe)
    ...
    protected fun ByteArray.calculateCrc8() =
        fold(0) { sum, it -> crc8[(sum xor it.toInt()).let { if (it < 0) it + 256 else it }] }
}
